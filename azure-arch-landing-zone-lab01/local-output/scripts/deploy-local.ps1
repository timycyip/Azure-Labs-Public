param(
  [switch]$destroy,
  [string]$root_module_folder_relative_path = ".",
  [string]$remote_state_resource_group_name = "rg-alz-mgmt-state-eastus2-001",
  [string]$remote_state_storage_account_name = "stoalzmgmeas001imrb",
  [string]$remote_state_storage_container_name = "mgmt-tfstate",
  [switch]$auto_approve
)
...
$initResult = & $command $arguments *>&1
$initResult | Write-Host
...
Write-Host "Running: $command $arguments"
$planResult = & $command $arguments *>&1
$planResult | Write-Host
if ($LASTEXITCODE -ne 0) {
    Write-Error "Terraform plan failed with exit code $LASTEXITCODE. The plan file might be incomplete."
    exit $LASTEXITCODE
}
